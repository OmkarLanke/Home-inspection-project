...
def get_gemini_response(question: str, context: str) -> str:
    try:
        ...
        prompt = (
            ...
            f"Provide a detailed, practical solution in JSON format, focusing on step-by-step instructions to address the issue. "
            ...
            f"If professional help is needed, explicitly state so and explain why. The response should be a valid JSON object with the following structure:\n"
            f'{{"steps": [{{"step": 1, "instruction": "string"}}, ...]}}'
        )
        ...
        # Convert JSON to human-readable text with specified steps
        formatted_response = ""
        if "steps" in response_json:
            steps = response_json["steps"]
            # Filter and reorder steps based on provided input (3, 6, 7, 9)
            selected_steps = []
            safety_guidelines = []
            for step in steps:
                instruction = step.get("instruction", "").strip()
                if instruction:
                    if any(note in instruction for note in ["Important Note", "Assess the situation", "Safety First", "Professional Help", "Tools and Materials", "Post-removal"]):
                        safety_guidelines.append(instruction.replace("Important Note: ", "").strip())
                    elif "3. Choose a time to work" in instruction:
                        selected_steps.append(("3. Choose a time to work", "Early morning or late evening when bees are less active is ideal. Avoid working during rain or high winds."))
                    elif "6. Remove the nest" in instruction:
                        selected_steps.append(("6. Remove the nest (DIY only for small, accessible nests)", "Carefully approach the nest. Use the brush or broom to gently knock the nest into a plastic bag. Seal the bag immediately. If using insecticide, follow the manufacturer's instructions carefully and only as a last resort. Dispose of the bag responsibly according to local regulations."))
                    elif "7. Clean up" in instruction:
                        selected_steps.append(("7. Clean up", "Remove any remaining debris. Seal any gaps or cracks where the bees/wasps entered to prevent future infestations. Consider installing pest-resistant materials around the window frame."))
                    elif "9. Repair and Maintenance" in instruction:
                        selected_steps.append(("9. Repair and Maintenance", "Assess the damage to the brickwork and window frame. Repair or replace damaged sections as needed. Refer to Australian Standard AS 1668.1-2018 for guidance on building fabric maintenance and repair."))
            
            # Add safety guidelines as bolded text at the top
            if safety_guidelines:
                formatted_response += "\n".join(f"<strong>{guideline}</strong>" for guideline in safety_guidelines) + "\n\n"
            
            # Number the selected steps sequentially
            step_num = 1
            for _, instruction in selected_steps:
                formatted_response += f"{step_num}. {instruction}\n"
                step_num += 1
        else:
            formatted_response = "Sorry, I couldn't generate a proper step-by-step response. Please try asking again."
        
        # Improvements: Add intro and safety reminder
        formatted_response = f"Here’s a simple guide to remove the nest safely:\n\n{formatted_response}\n\nAlways prioritize your safety and consider professional help if unsure!"
        
        return formatted_response.strip()
    except Exception as e:
        return f"Sorry, there was an error generating the response: {str(e)}. Please try again or check your internet connection."
...
"""
        ...
        if st.session_state.chat_history:
            ...
            for message in st.session_state.chat_history:
                ...
                if message["role"] == "bot":
                    # Display the response as a numbered list
                    html_content = "<ol>" + "".join(f"<li>{line.strip()}</li>" for line in message["content"].split("\n") if line.strip() and not line.startswith("<strong>"))
                    # Add any important notes or disclaimers outside the list
